...
public class DrawBoardView extends View {
     ...
     * 是否圆角
     */
    private Paint.Cap paintStrokeCap = Paint.Cap.SQUARE;
    /**
     * 是否有模糊效果
     */
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        lineStrokeWidth = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 15f, displayMetrics);
        eraserStrokeWidth = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 30f, displayMetrics);
    ...
    private Paint createPaint(int drawMode) {
        ...
        if (drawMode == DrawMode.ERASER) { // 当为橡皮擦模式时
            ...
            paint.setStrokeCap(paintStrokeCap);
            if(isBlurMask){
                paint.setMaskFilter(new BlurMaskFilter(50f, BlurMaskFilter.Blur.SOLID));
            }
            if (paintShader != null) {
     ...
     * 设置是否为圆角
     */
    public void setRound(@NonNull Paint.Cap paintStrokeCap) {
     ...
     * 设置是否为模糊
     */
    public void setBlurMask(boolean isBlurMask) {
